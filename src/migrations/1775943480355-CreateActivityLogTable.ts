...
export class CreateActivityLogTable1775943480355 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`transactions\` DROP FOREIGN KEY \`FK_b1630a0b9be02ae4be5dff17821\``);
        ...
        await queryRunner.query(`CREATE TABLE \`activity_logs\` (\`id\` varchar(36) NOT NULL, \`adminId\` varchar(255) NULL, \`action\` varchar(255) NOT NULL, \`details\` json NULL, \`ipAddress\` varchar(255) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`transactions\` DROP COLUMN \`disputeId\``);
        await queryRunner.query(`ALTER TABLE \`transactions\` DROP COLUMN \`disputesId\``);
        ...
        await queryRunner.query(`ALTER TABLE \`admins\` ADD UNIQUE INDEX \`IDX_051db7d37d478a69a7432df147\` (\`email\`)`);
